...
def _output_dir_for_video(
    ...
    if preserve_subdirs:
        ...
        return output_dir / parent_rel / safe_stem
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Split all videos in an input folder into frames in an output folder (organized by video), then archive processed videos."
    )
    parser.add_argument(
        "-i",
        "--input",
        default="input",
        help="Input folder containing videos (default: ./input)",
    )
    parser.add_argument(
        "-o",
        "--output",
        default="output",
        help="Output folder for frames (default: ./output)",
    )
    parser.add_argument(
        "-a",
        "--archive",
        default="archive",
        help="Archive folder for processed videos (default: ./archive)",
    )
    parser.add_argument(
        "--no-recursive",
        action="store_true",
...
def main(argv: list[str]) -> int:
    ...
    for video_path in videos:
        ...
        if args.dry_run:
            print("dry-run: would extract frames and archive")
            continue
